    ...
    <div className="relative" ref={boxRef}>
      <div className="relative">
        ...
        <input
          ...
          disabled={loading}
          className="w-full rounded-xl bg-black/40 border border-[var(--accent-2)]/30 pl-11 pr-4 py-3 text-sm outline-none focus:border-[var(--accent-2)]/70 transition placeholder:text-[var(--muted)] disabled:opacity-60"
        />
        {(searching || loading) && (
          <span className="absolute right-4 top-1/2 -translate-y-1/2 spin inline-block w-4 h-4 border-2 border-white/30 border-t-white rounded-full" />
        )}
      </div>

      {open && (results.length > 0 || searching) && (
        <div className="absolute z-30 mt-2 w-full rounded-xl bg-[#13111c] border border-white/10 shadow-2xl overflow-hidden max-h-80 overflow-y-auto">
          ...
          {results.map((t) => (
            ...
            >
              {t.albumArt ? (
                ...
                <span className="w-10 h-10 rounded bg-white/10" />
              )}
